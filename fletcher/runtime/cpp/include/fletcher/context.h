...
class Context {
 public:
   /**
    * @brief Fletcher action reg offset for multiple instance supports 
    */
  uint64_t fletcher_snap_action_reg_offset = 0ul;
  /**
   * @brief Context constructor.
   * @param[in] platform  A platform to construct the context on.
   */
  //explicit Context(std::shared_ptr<Platform> platform) : platform_(std::move(std::atomic_load(&platform))) {}
  explicit Context(std::shared_ptr<Platform> platform) //: platform_(std::move(std::atomic_load(&platform))) {
  {
    std::atomic_store(&platform_, platform); 
  }
  ...
  inline std::shared_ptr<Platform> platform() const { return std::atomic_load(&platform_); }
