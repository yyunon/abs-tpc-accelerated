...
Status Kernel::Reset() {
  auto context_l = std::atomic_load(&context_);
  auto status = context_l->platform()->WriteMMIO(context_->fletcher_snap_action_reg_offset, FLETCHER_REG_CONTROL, ctrl_reset);
  if (status.ok()) {
    return context_l->platform()->WriteMMIO(context_->fletcher_snap_action_reg_offset, FLETCHER_REG_CONTROL, 0);
  } else {
    return status;
  }
}
...
Status Kernel::SetRange(size_t recordbatch_index, int32_t first, int32_t last) {
  ...
  Status ret;
  if (!context_->platform()->WriteMMIO(context_->fletcher_snap_action_reg_offset, FLETCHER_REG_SCHEMA + 2 * recordbatch_index,
                                       static_cast<uint32_t>(first)).ok()) {
    ret = Status::ERROR();
  }
  if (!context_->platform()->WriteMMIO(context_->fletcher_snap_action_reg_offset, FLETCHER_REG_SCHEMA + 2 * recordbatch_index + 1,
...
Status Kernel::SetArguments(const std::vector<uint32_t>& arguments) {
  for (int i = 0; (size_t) i < arguments.size(); i++) {
    context_->platform()->WriteMMIO(
        context_->fletcher_snap_action_reg_offset, FLETCHER_REG_SCHEMA + 2 * context_->num_recordbatches() + 2 * context_->num_buffers() + i, arguments[i]);
  }
...
Status Kernel::Start() {
  ...
  status = context_->platform()->WriteMMIO(context_->fletcher_snap_action_reg_offset, FLETCHER_REG_CONTROL, ctrl_start);
  if (!status.ok())
    return status;
  return context_->platform()->WriteMMIO(context_->fletcher_snap_action_reg_offset, FLETCHER_REG_CONTROL, 0);
}

Status Kernel::GetStatus(uint32_t *status_out) {
  return context_->platform()->ReadMMIO(context_->fletcher_snap_action_reg_offset, FLETCHER_REG_STATUS, status_out);
}

Status Kernel::GetReturn(uint32_t *ret0, uint32_t *ret1) {
  Status status;
  status = context_->platform()->ReadMMIO(context_->fletcher_snap_action_reg_offset ,FLETCHER_REG_RETURN0, ret0);
  if ((ret1 == nullptr) || (!status.ok())) {
    return status;
  }
  status = context_->platform()->ReadMMIO(context_->fletcher_snap_action_reg_offset, FLETCHER_REG_RETURN1, ret1);
  return status;
}
...
Status Kernel::WaitForFinish(unsigned int poll_interval_usec) {
  ...
  if (poll_interval_usec == 0) {
    do {
      context_->platform()->ReadMMIO(context_->fletcher_snap_action_reg_offset, FLETCHER_REG_STATUS, &status);
...
Status Kernel::WriteMetaData() {
  ...
  for (size_t i = 0; i < context_->num_recordbatches(); i++) {
    ...
    status = platform->WriteMMIO(context_->fletcher_snap_action_reg_offset,offset, 0);               // First index
    if (!status.ok()) return status;
    offset++;
    status = platform->WriteMMIO(context_->fletcher_snap_action_reg_offset,offset, rb->num_rows());  // Last index (exclusive)
    if (!status.ok()) return status;
    offset++;
  }
  ...
  for (size_t i = 0; i < context_->num_buffers(); i++) {
    ...
    // Write the address
    platform->WriteMMIO(context_->fletcher_snap_action_reg_offset,offset, address.lo);
    if (!status.ok()) return status;
    offset++;
    platform->WriteMMIO(context_->fletcher_snap_action_reg_offset,offset, address.hi);
    if (!status.ok()) return status;
    offset++;
  }
