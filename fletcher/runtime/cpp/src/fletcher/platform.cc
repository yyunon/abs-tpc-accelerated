...
Status Platform::ReadMMIO64(uint64_t fletcher_snap_action_reg_offset ,uint64_t offset, uint64_t *value) {
  freg_t hi, lo;
  Status stat;

  // Read high bits
  stat = ReadMMIO(fletcher_snap_action_reg_offset, offset + 1, &hi);
  if (!stat.ok()) {
    return stat;
  }
  *value = ((uint64_t) hi) << 32u;
  ...
  // Read low bits
  stat = ReadMMIO(fletcher_snap_action_reg_offset, offset, &lo);
  if (!stat.ok()) {
    return stat;
  }
  *value |= lo;
...
Status Platform::MmioToString(uint64_t fletcher_snap_action_reg_offset, std::string *str, uint64_t start, uint64_t stop, bool quiet) {
  std::stringstream ss;
  Status stat;
  for (uint64_t off = start; off < stop; off++) {
    uint32_t val;
    stat = ReadMMIO(fletcher_snap_action_reg_offset, off, &val);
    if (!stat.ok()) {
      return stat;
    } else {
      if (!quiet) {
